    ...
    <tbody>
      <tr>
		...
		include 'connect.php';
							...
							include 'connect.php';
								$id = urldecode($_GET['ct']);
								$sql1 = "SELECT DISTINCT restaurantid FROM menutbl WHERE cuisinetype='$id'";
								...
								if($num1>=1){
									...
									{
										$restroid=$row1["restaurantid"];
										$sql="SELECT DISTINCT restaurantname,resturantid FROM restauranttbl WHERE resturantid=$restroid";
										...
										if($num>=1){
											...
											{
												echo '<li><a href="userInterface.php?id='.urldecode($row['resturantid']).'">'.$row["restaurantname"].'</a></li>';
